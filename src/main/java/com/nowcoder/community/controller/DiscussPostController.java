    ...
    public String getDiscussPost(@PathVariable("discussPostId") int discussPostId, Model model, Page page) {
        ...
        if (commentList != null) {
            for (Comment comment : commentList) {
                ...
                //点赞
                likeCount = likeService.findEntityLikeCount(ENTITY_TYPE_COMMNENT, comment.getId());
                commentVO.put("likeCount", likeCount);
                likeStatus = hostHolder.getUser() == null ? 0 :
                        likeService.findEntityLikeStatus(hostHolder.getUser().getId(), ENTITY_TYPE_COMMNENT, comment.getId());
                ...
                if (replyList != null) {
                    for (Comment reply : replyList) {
                        ...
                        //点赞
                        likeCount = likeService.findEntityLikeCount(ENTITY_TYPE_COMMNENT, reply.getId());
                        replyVO.put("likeCount", likeCount);
                        likeStatus = hostHolder.getUser() == null ? 0 :
                                likeService.findEntityLikeStatus(hostHolder.getUser().getId(), ENTITY_TYPE_COMMNENT, reply.getId());
